...
"""
    ...
    def replace_bullets_exactly(section_title, new_bullets):
        for i in range(len(doc.paragraphs)):
            if section_title in doc.paragraphs[i].text:
                j = i + 1
                # Remove bullets until we hit a non-bullet non-empty line
                while j < len(doc.paragraphs) and (doc.paragraphs[j].text.strip().startswith("•") or doc.paragraphs[j].text.strip() == ""):
                    del doc.paragraphs[j]
                # Insert new bullets directly as plain text
                for idx, bullet in enumerate(new_bullets):
                    doc.paragraphs.insert(j + idx, doc.add_paragraph(bullet))
                break

    replace_bullets_exactly("UNIVERSITY OF ILLINOIS URBANA-CHAMPAIGN", experience[0:2])
    replace_bullets_exactly("EXTUENT", experience[2:5])
    replace_bullets_exactly("FRAPPE", experience[5:10])
